<template>
    <div class="game-wrap">
        ...
        <div :class="['layer-pk-mode','layer-pk-mode-level',isPkWin?'success':'fail']" v-if="gameStatus == 'end'">
            <div class="gameover-result-title">{{isPkWin?"闯关成功":"闯关失败"}}</div>
            <div class="gameover-result-content">
                ...
                <div class="gameover-btns">
                    ...
                    <a class="try-again" 
                    @click="oneMoreTry()" 
                    v-if="gameStatus == 'end' && (questionIndex==beginData.length-1 || (questionIndex<beginData.length-1 && !isPkWin))" 
                    >再来一局</a>
                    <a class="try-again" @click="nextLevel()" v-if="gameStatus == 'end' && questionIndex<beginData.length-1 && isPkWin" >下一关</a>
                </div>
            </div>
    ...
    export default {
        data() {
            return {
                ...
                mode : this.$route.query.mode || 'level',      //模式
        ...
        methods: {
            ...
            */
            pkModeBegin(index){
                ...
                this.questionTimer = setTimeout(()=>{
                    ...
                    var index = Math.floor(this.questionIndex/5);
                    this.ruleitem = this.rule[index];
                    ...
                    this.timingCountDown(this.ruleitem.max_duration);
                },1500)
            },
            playAudio(){
                if(!this.isIOS){
            ...
            */
            checkAnswer(){
                ...
                this.isRotate = false;
                this.questionReply();
                if(this.questionIndex%5==4){
                    setTimeout(()=>{
                        this.gameStatus = "end";
                        this.isPkWin = this.correctCount>=3;
                        var index = Math.ceil(this.questionIndex/5);
                        var rule = this.rule[index];
                        this.score = this.correctCount * rule.level_score;
                    },2000)
                }else{
